...
struct Circle {
	...
	//check this
	static Circle inscribedCircle(point<T> p1, point<T> p2, point<T> p3) {
		T a = p2.dist(p3), b = p1.dist(p3), c = p1.dist(p2);
		return Circle((a * p1.x + b * p2.x + c * p3.x) / (a + b + c), (a * p1.y + b * p2.y + c * p3.y) / (a + b + c), sss_area(a, b, c) / (a + b + c));
	}
